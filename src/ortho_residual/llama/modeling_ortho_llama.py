...
#     # f_x_fp32 = f_x.to(torch.float32)  # for numerical stability
#     # with torch.no_grad():
#     #     print("x:", x.min(), x.max(), x.mean(), x.std())
#     #     print("f_x:", f_x.min(), f_x.max(), f_x.mean(), f_x.std())
#     #     tmp = (x * f_x).sum(dim, keepdim=True)
#     #     print("dot:", tmp.min(), tmp.max(), tmp.mean(), tmp.std())
    
#     dot      = (x * f_x).sum(dim, keepdim=True)
#     norm_x2  = (x * x  ).sum(dim, keepdim=True).to(eps.dtype) + eps # numerical stability, and explicitly cast to eps dtype
...
#     norm_sq  = (x_view * x_view).sum(dim=dim, keepdim=True).to(eps.dtype) + eps # numerical stability, and explicitly cast to eps dtype
...
#     def __init__(self, config: OrthoLlamaConfig, device=None):
#         super().__init__()
...
class OrthoLlamaDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: OrthoLlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
@auto_docstring
class OrthoLlamaPreTrainedModel(PreTrainedModel):
...
@auto_docstring
class OrthoLlamaModel(OrthoLlamaPreTrainedModel):
    config_class = OrthoLlamaConfig
    def __init__(self, config: OrthoLlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [OrthoLlamaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
@auto_docstring
class OrthoLlamaForCausalLM(OrthoLlamaPreTrainedModel, GenerationMixin):
    config_class = OrthoLlamaConfig
